...
class LearnAnywhereApp:
    ...
    def export_results(self):
        """Export quiz results to CSV with enhanced data"""
        try:
            cursor = self.conn.cursor()
            cursor.execute("""
                SELECT student_name, grade, subject, score, total_questions, wrong_answers, date_taken
                FROM quiz_results
            ...
            if not results:
                self.show_snackbar("No quiz results to export!", ft.colors.ORANGE)
                return
            ...
            with open(filename, 'w', newline='', encoding='utf-8') as file:
                ...
                writer.writerow([
                    "Student Name", "Grade", "Subject", "Score", "Total Questions", 
                    "Percentage", "Wrong Questions Count", "Date Taken"
                ])
                
                for result in results:
                    name, grade, subject, score, total, wrong_answers_str, date = result
                    percentage = f"{(score/total)*100:.1f}%"
                    
                    # Count wrong answers
                    wrong_count = 0
                    if wrong_answers_str:
                        try:
                            wrong_answers = eval(wrong_answers_str)
                            wrong_count = len(wrong_answers)
                        except:
                            wrong_count = total - score
                    
                    writer.writerow([name, grade, subject, score, total, percentage, wrong_count, date])
            ...
            dialog = ft.AlertDialog(
                ...
                content=ft.Text(f"Results exported to: {filename}\n\nThe file has been saved in the application folder."),
            ...
            self.show_snackbar(f"Failed to export results: {str(e)}", ft.colors.RED)
